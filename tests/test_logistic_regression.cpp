...
struct LogisticRegressionTester {
  static double sigmoid(const LogisticRegression& m, double z) {
    return m.sigmoid(z);
  }
  static Vector& weights(LogisticRegression& m) { return m.weights; }
  static double& bias(LogisticRegression& m) { return m.bias; }
  static void updateWeights(LogisticRegression& m, const ProcessedData& data,
                            double alpha) {
    m.updateWeights(data, alpha);
  }
};
...
TEST_CASE("SIGMOID PRODUCES VALUES BETWEEN 0 AND 1", "[model]") {
  ...
  REQUIRE(LogisticRegressionTester::sigmoid(model, 0.0) ==
          Approx(0.5).margin(1e-5));
  REQUIRE(LogisticRegressionTester::sigmoid(model, 100.0) ==
...
TEST_CASE("UPDATEWEIGHTS ADJUSTS WEIGHTS IN THE RIGHT DIRECTION", "[model]") {
  ...
  REQUIRE(LogisticRegressionTester::weights(model)[0] ==
          Approx(0.05749).margin(0.001));
  REQUIRE(LogisticRegressionTester::bias(model) ==
...
TEST_CASE("PERSISTENCE SAVE/LOAD CHECK", "[model]") {
  ...
  REQUIRE(LogisticRegressionTester::bias(loaded_model) ==
          Approx(original_bias));
  REQUIRE(LogisticRegressionTester::weights(loaded_model).size() ==
          original_weights.size());
  for (size_t i = 0; i < LogisticRegressionTester::weights(loaded_model).size();
       ++i) {
    REQUIRE(LogisticRegressionTester::weights(loaded_model)[i] ==
            Approx(original_weights[i]));
  }
