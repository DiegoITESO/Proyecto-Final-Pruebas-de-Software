...
class ArgParser {
 ...
 private:
  ...
  void handlePredict();
  void handleEvaluate();
  void saveToCSVFile(std::vector<double>& predictions,
                     std::string destinationPath);
  void collectTrainData(std::string& filePath, std::string& destinationPath,
                        bool& hasHeader, size_t& churnColumn,
                        std::vector<size_t>& dropColumns, double& alpha,
                        int& epochs);
  void collectPredictData(std::string& filePath, std::string& weightsPath,
