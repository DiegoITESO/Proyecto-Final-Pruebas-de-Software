  ...
  while (std::getline(file, line)) {
    ...
    if (data.size() && (row.size() != data[0].size()))
      throw std::invalid_argument(
          "The number of columns in the file is inconsistent");
    data.push_back(row);
  }
  return data;
}
ProcessedData CSVReader::preprocess(
...
// PARAMETERS~~~~//
{
  ...
  for (const std::vector<std::string>& row : data) {
    ...
    for (size_t i = 0; i < row.size(); ++i) {
      ...
      if (!StringHandling::isNumber(row[i]) && lowercaseString != "yes" &&
          lowercaseString != "no")
  ...
  /*
     ...
     ELIMINATED THUS, THE PURPOSE OF THE OFFSET IS TO SKIP THOSE COLUMNS WHEN
  ...
  */
  ...
  for (const auto& row : data) {
    ...
    std::vector<double> processedRow(
        newSize);  // VALUE OF EACH OF THE NEW PARAMETERS
    offset = 0;
    for (size_t i = 0; i < row.size(); ++i) {
      std::string lowercaseString =
          StringHandling::toLower(row[i]);  // STANDARDIZE THE STRINGS
      if (i == churnColumn) {
        if (lowercaseString == "yes")
          churnResults.push_back(1.0);
        else if (lowercaseString == "no")
          churnResults.push_back(0.0);
        else {
          ...
          std::cout << std::endl;
          throw std::invalid_argument(
              "Churn results must be expressed as yes/no values. Value found: "
              "'" +
              lowercaseString + "'");
        }
        offset++;
      ...
      if (lowercaseString == "yes")
        processedRow[i - offset] = 1.0;
      else if (lowercaseString == "no")
        processedRow[i - offset] = 0.0;
...
void CSVReader::normalizeFeatures(ProcessedData& data) {
  ...
  for (size_t j = 0; j < numFeatures; ++j) {
    for (size_t i = 0; i < numSamples; ++i) means[j] += data.features[i][j];
    means[j] /= numSamples;
  }
  for (size_t j = 0; j < numFeatures; ++j) {
    for (size_t i = 0; i < numSamples; ++i)
      stddevs[j] +=
          (data.features[i][j] - means[j]) * (data.features[i][j] - means[j]);
